      ...
      try {
        ...
        if (
          ...
          const timestamp = timestampStr
            ? new Date(timestampStr as string | number | Date).getTime()
            : Date.now();
  ...
  const { isConnected } = useJobSSE<AnalysisResultData>(
    ...
    aiService.getProjectStatusStreamUrl,
    {
      ...
      onMessage: (data) => {
        ...
        // New logic from the instruction, assuming 'data' refers to 'event' here
        if (event.status === "completed" || event.status === "success") {
          ...
          if (currentJobType === "image" && result) {
            // Handle image result...
          } else if (currentJobType === "analysis" && result) {
            // Additional check if needed, but primary handling is in useEffect
          }
        }
      ...
      onError: (err) => {
        ...
        setBufferAnalyzing(false);
        onErrorRef.current?.("Analysis failed");
      },
    },
  );
    ...
    try {
      for (const doc of changedDocuments) {
        ...
        const jobId =
          actualData?.jobId ||
          actualData?.id ||
          (resData as { documentId?: string }).documentId;
