...
export function RelationshipDetailSheet({
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent className="w-[400px] sm:w-[540px] p-0 border-l border-cloud-200">
        <ScrollArea className="h-full">
          <div className="p-6 space-y-8">
            ...
            <SheetHeader className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                {bidirectional && (
                  <Badge
                    intent="secondary"
                    className="gap-1 bg-cloud-100 text-mocha-600 hover:bg-cloud-200"
                  >
              ...
              <SheetTitle className="text-2xl  font-bold text-espresso-900 leading-tight">
                <span className="block text-mocha-500 text-base font-sans font-medium mb-1">
                  Source
                </span>
                {sourceName || "Unknown"}
                <span className="block my-2 text-mocha-200 text-sm border-b border-dashed" />
                <span className="block text-mocha-500 text-base font-sans font-medium mb-1">
                  Target
                </span>
                {targetName || "Unknown"}
            ...
            {since && (
              <div className="bg-amber-50 border border-amber-200 rounded-md p-4 flex items-center gap-3 shadow-sm">
                ...
                <div>
                  ...
                  <p className="text-espresso-800 font-medium">{since}</p>
                </div>
              </div>
            )}
            ...
            <div className="space-y-3">
              <h4 className="text-sm font-semibold text-mocha-500 uppercase tracking-wider">
                관계 설명
              </h4>
              {relationLabel || description ? (
                <p className="text-espresso-800 leading-relaxed whitespace-pre-wrap font-sans text-base">
                  {relationLabel || description}
                </p>
              ) : (
                <p className="text-mocha-400 italic text-sm">
                  관계에 대한 설명이 없습니다.
                </p>
              )}
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs text-mocha-400">Low</span>
                  <span className="text-lg font-bold text-mocha-500">
                    {strength} / 10
                  </span>
                  <span className="text-xs text-mocha-400">High</span>
                </div>
              </div>
              <div className="h-3 w-full bg-cloud-100 rounded-full overflow-hidden shadow-inner">
                <div
            ...
            <div className="space-y-4">
              <h4 className="text-sm font-semibold text-mocha-500 uppercase tracking-wider flex items-center gap-2">
                <Activity className="w-4 h-4" />
                관계 변천사
              </h4>
              {history && Array.isArray(history) && history.length > 0 ? (
                <div className="relative pl-4 space-y-6 before:content-[''] before:absolute before:left-[7px] before:top-2 before:bottom-2 before:w-[2px] before:bg-cloud-200">
                  ...
                  {history.map((event: any, idx: number) => {
                    ...
                    return (
                      <div key={event.eventId || idx} className="relative">
                        ...
                        <div
                          className={cn(
                            "absolute -left-[13px] top-1.5 w-3 h-3 rounded-full border-2 border-white ring-1 ring-cloud-200",
                          )}
                          style={{ backgroundColor: eventColorHex }}
                        />

                        <div className="bg-white p-3 rounded-lg border border-cloud-100 shadow-sm hover:shadow-md transition-shadow">
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-xs font-bold text-mocha-500 bg-cloud-100 px-2 py-0.5 rounded">
                          ...
                          <h5 className="font-semibold text-espresso-800 text-sm mb-1">
                            {event.title}
                          </h5>
                          {event.reason && (
                ...
                <p className="text-sm text-mocha-400 italic">
            ...
            {evolvedFrom && (
              <div className="p-4 bg-cloud-50 rounded-lg border border-cloud-100 space-y-2">
                <div className="flex items-center gap-2 text-mocha-500 mb-2">
                  <Activity className="w-4 h-4" />
                  <span className="text-sm font-medium">이전 관계</span>
                </div>
                <div className="flex items-center gap-3">
                  <Badge intent="outline" className="text-mocha-500 bg-white">
