...
export default function EditorLeftSidebar({
  ...
  // CLS 방지: 항상 동일한 DOM 구조 유지, width만 변경
  // isOpen이 false면 40px 고정, true면 resizable width 사용
  const sidebarWidth = isOpen ? width : 40;

  return (
    <motion.aside
      className={cn(
        "hidden md:flex shrink-0 overflow-hidden relative group/sidebar z-20",
      ...
      aria-hidden={!isOpen}
    >
      {/* Collapsed State - 40px strip with toggle button */}
      {!isOpen && (
        <div className="w-full h-full flex flex-col items-center py-3 border-r border-mocha-100 bg-cloud-50">
          <motion.button
            onClick={() => onToggle?.()}
