...
export function SelectionStep({
  ...
  return (
    <div className="flex gap-4 h-[500px]">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="relative mb-3">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-cloud-50 border border-stone-200 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-mocha-400/30 focus:border-mocha-400"
          />
        </div>
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          />
          <span className="text-sm text-stone-600">미배포만 보기</span>
        </div>
          ...
          {filteredDocuments.map((doc, index) => (
            <label
              ...
              className={cn(
                ...
                doc.isPublished
                  ? "opacity-60 cursor-not-allowed bg-stone-50 border-stone-200" // 배포됨: 비활성화 스타일
                  : selectedIds.includes(doc.id)
                    ? "border-mocha-400 bg-mocha-50/50 cursor-pointer"
                    : "border-stone-200 hover:border-mocha-300 bg-white cursor-pointer",
              )}
            >
              <input
                type="checkbox"
                ...
                className={cn(
                  "w-4 h-4 rounded border-stone-300 text-mocha-500 focus:ring-mocha-400",
      ...
      <div className="w-72 flex flex-col bg-cloud-50 rounded-2xl p-4 border border-stone-200">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-bold text-stone-500 uppercase tracking-wide">
            배포 대기열
          </h3>
        ...
        <div className="flex-1 overflow-y-auto space-y-2">
          {selectedDocuments.length === 0 ? (
            ...
            selectedDocuments.map((doc, index) => (
              <div
                key={`${doc.id}-${index}`}
                className="flex items-center gap-2 p-2 bg-white rounded-xl border border-stone-200"
              >
                ...
                <button
                  ...
                  className="text-stone-400 hover:text-status-error"
                >
        ...
        <div className="mt-3 pt-3 border-t border-stone-200">
          <div className="flex justify-between text-sm">
            <span className="text-stone-500">선택된 섹션</span>
