...
4.  **PR 상태 확인 및 생성**:
    - `gh pr view --json state` 실행하여 현재 브랜치의 PR 상태 확인
    - **PR이 없거나 MERGED/CLOSED 상태인 경우**: 새 PR 생성
      ```
      gh pr create --title "생성된_메시지" --body "## 변경 사항\n- (변경 내용 한글 요약)\n\n## AI 리뷰\nAI 코드 리뷰가 자동으로 실행됩니다."
      ```
    - **PR이 OPEN 상태인 경우**: 기존 PR에 푸시됨을 안내
    - **중요**: PR 생성/업데이트 시 AI 코드 리뷰어가 자동으로 실행됩니다.
