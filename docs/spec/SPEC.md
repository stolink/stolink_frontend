...
> 🔧 기술 스택 → [TECHSTACK.md](../architecture/TECHSTACK.md)
...
| 4   | **데모 에디터** | `/demo`                  | 로그인 없이 에디터 체험     |
| 5   | 에디터          | `/projects/:id/editor`   | 글쓰기, 복선 관리, AI 챗봇  |
| 6   | 세계관          | `/projects/:id/world`    | 캐릭터 관계도, 장소, 아이템 |
| 7   | 작품 설정       | `/projects/:id/settings` | 내보내기, 공유, 통계        |
...
**경로:** `/projects/:id/editor` (프로젝트) / `/demo` (데모)
...
| 모드            | 설명                  | 트리거           |
| --------------- | --------------------- | ---------------- |
| **Single**      | 단일 문서 편집        | 기본 모드        |
| **Scrivenings** | 여러 문서 통합 편집   | 폴더 선택 시     |
| **Outline**     | 테이블 기반 아웃라인  | 툴바 버튼        |
| **Corkboard**   | 카드 기반 시놉시스 뷰 | 툴바 버튼 (예정) |
...
**폴더:** `src/components/editor/sidebar/`

| 컴포넌트          | 역할                             |
| ----------------- | -------------------------------- |
| `ChapterTree.tsx` | 메인 트리 컨테이너               |
| `TreeItem.tsx`    | 개별 노드 (클릭/더블클릭/우클릭) |
| `ContextMenu.tsx` | 재사용 우클릭 메뉴               |
| `NodeIcon.tsx`    | 타입별 아이콘                    |
| `types.ts`        | ChapterNode 타입 정의            |
...
- 범위: 50% - 200%
- 슬라이더 또는 +/- 버튼으로 조절
- `TiptapEditor.tsx`에서 `useState(100)`으로 관리
...
> ⭐ 아래 기능들은 에디터 완성도 향상을 위한 **향후 구현 예정 기능**입니다.

| 기능              | 설명                                              | 우선순위 |
| ----------------- | ------------------------------------------------- | -------- |
| **타자기 모드**   | 현재 입력 라인을 화면 중앙에 고정, 시선 피로 감소 | P1       |
| **포커스 모드**   | 주변 UI 숨김, 현재 문단만 강조                    | P1       |
| **스플릿 뷰**     | 에디터 분할하여 다른 문서 동시 참조               | P1       |
| **인스펙터 패널** | 현재 문서의 메타데이터, 메모, 연결 정보 표시      | P2       |
| **스냅샷**        | 특정 시점 저장, Diff 비교, 롤백 기능              | P2       |
| **플랫폼 프리셋** | 문피아/조아라 스타일 내보내기 설정                | P3       |

**스플릿 뷰 구현 방향:**

- **하이브리드 형태** = 인스펙터(고정 패널) + 멀티 페인(분할 에디터)
- 인스펙터: 현재 씬 관련 캐릭터, 복선, 설정 자동 표시
- 멀티 페인: 사용자가 원하는 문서 동시 열기

### 5.11 자연어 → 데이터베이스 변환 파이프라인 ⭐

> **[핵심 기능]** StoLink의 차별화 요소로, 본문 텍스트에서 캐릭터, 관계, 장소, 아이템 등을 **AI가 자동 추출**하여 구조화된 데이터베이스로 변환합니다.
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│   [에디터 본문]  →  [AI 분석 엔진]  →  [구조화 데이터]  →  [UI 연동]    │
│                                                                         │
│   "아린이 검을       LangGraph +       Character: 아린                  │
│    전설의검을        LLM 추출          Item: 전설의검    →  관계도 노드  │
│    받았다"                             Relation: 소유       아이템 목록  │
│                                                             복선 태그   │
└─────────────────────────────────────────────────────────────────────────┘
```
...
- **버퍼 임계치**: 글자 수 5,000자 이상 (자동 플러시 비활성인 경우 사용자 수동 트리거)
- **시간 제한**: 마지막 분석 후 30분 경과
- **이탈**: 페이지 닫기, 브라우저 종료 시 (`beforeunload`)
- **중복 방지**: Content Hash 비교를 통해 변경된 문서만 선별 분석

**추출 대상 엔티티:**

| 엔티티     | 추출 정보                        | 저장 위치          |
| ---------- | -------------------------------- | ------------------ |
| **캐릭터** | 이름, 역할, 성격, 외형           | PostgreSQL + Neo4j |
| **관계**   | 소스-타겟, 유형(친밀/적대), 강도 | Neo4j              |
| **장소**   | 이름, 유형, 연관 캐릭터          | PostgreSQL         |
| **아이템** | 이름, 능력, 소유자, 소유 이력    | PostgreSQL         |
| **복선**   | 태그, 등장 위치, 회수 여부       | PostgreSQL         |

**데이터 연동 UI (향후):**

- **호버 카드**: 캐릭터명에 마우스 오버 시 상세 정보 팝업
- **멘션 자동완성**: `@캐릭터명` 입력 시 기존 캐릭터 추천
- **상태창 블록**: LitRPG용 시스템 메시지 스타일 블록 서식

### 5.12 게이미피케이션 (향후)

| 요소          | 설명                                 | 우선순위 |
| ------------- | ------------------------------------ | -------- |
| **히트맵**    | 날짜별 집필량 시각화 (GitHub 스타일) | P3       |
| **스트릭**    | 연속 집필 일수 카운트                | P3       |
| **목표 관리** | 일일/주간/프로젝트 단어 수 목표      | P3       |
| **배지**      | 목표 달성 시 디지털 배지 부여        | P3       |
...
- **그래프 시각화 (Canvas 2D 가속):**
  - **D3 + Canvas 기반 엔진**: 1000+ 노드 지원 및 60fps 유지
...
| 구분       | 기술                                                |
| ---------- | --------------------------------------------------- |
| Frontend   | React 19.2, TypeScript 5.9, Tailwind CSS, shadcn/ui |
...
```
에디터 변경 → 디바운스 0.5초 → 델타 계산 → Spring API → PostgreSQL (델타 로그)
                                                    → S3 (10분마다 스냅샷)
```
...
```
사용자 입력 → IndexedDB 버퍼 (로컬) → (5,000자/30분) → Hash 차분 → SSE 플러시 → FastAPI
                                                     ↓
    PostgreSQL (NoSQL)에 즉시 병합 ← [분석 결과 스트림] ← LangGraph
          ↓
    SSE → 클라이언트 UI 업데이트 (실시간 반영)
```
...
| 1.7  | 2026.01.04 | 증분 분석 시스템(SSE + IndexedDB) 및 트리거 조건 명세 추가                           |
